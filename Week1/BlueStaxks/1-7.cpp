...
int solution(int n, vector<int> weak, vector<int> dist) //시계방향, 시계 반대방향,  뭐 다 의미없음
{
    ...
    for (i = 1; i <= dist.size(); ++i)
    {
        ...
        do {
            ...
            {
                zp = weak[j]; //제로 포인트를 맞춰 주는 것. 첫 데이터를 0으로 맞춰버리는 것임 -> 나머지는 zp를 뺀 값으로 하고 그게 음수일 수도 있으므로 +n하고 %n을 함
                p = k = 0;
                for (l = 0; l < tc.size(); ++l)
                {
                    c = 0;
                    ...
                    while ((weak[(k + j) % weak.size()] - zp + n) % n <= p) //커진 p이하의 weak포인트는 k로 다 쩨껴버림
                    {
                        ...
                        c = 1;
                        if (k == weak.size())
                            return l + 1; //사실 이건 tc.size()와 같게 됨. 근데 이게 더 안전한 느낌
                    }
